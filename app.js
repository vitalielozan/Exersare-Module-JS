...
vehiculForm.addEventListener('submit', (event) => {
  ...
  if (tip === 'masina') {
    if (isNaN(extra)) {
      ...
      flotaMea.addDataToJson(vehicul);
      flotaMea.adaugaVehicul(vehicul);
    } else {
      alert(
        'Valoarea introdusă pentru caroserie nu este validă. Introduceți date corecte!'
      );
    }
    ...
    if (!isNaN(extra)) {
      ...
      alert(
        'Valoarea introdusă pentru cilindri nu este validă. Introduceți date corecte!'
      );
    }
  } else {
...
btnRemove.addEventListener('click', async () => {
  ...
  await flotaMea.deleteAllVehicul();
  flotaMea.vehicule = [];
